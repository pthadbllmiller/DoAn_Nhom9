...
# =============================================================================
class Admin(RoomService, CustomerService, ContractService, BillService, EIndexService,RevenueService,QMainWindow, Ui_Main_Admin):
...
#================TIỆN ÍCH DÙNG CHUNG ===========================================================================================
    ...
    def load_table_json(self, table, filename, table_type):
        ...
        field_map = {
            ...
            "contract": ["maHD", "sophong", "khachthue", "trangthai"],
...
class fix_Contract(QMainWindow, Ui_MainWindow):
    def __init__(self, admin, contract_data=None, parent=None):
        super().__init__(parent)
        self.setupUi(self)
        self.admin = admin
        if contract_data:
            self.show_contract(contract_data)

    def show_contract(self, data):
        self.stackedWidget.setCurrentIndex(0)
        self.lne_mahopdong.setText(data["mahd"])
        self.lne_hoten_2.setText(data["makh"])
        self.lne_sophong.setText(data["phong"])
        self.date_ngaybd.setDate(QDate.fromString(data["ngaybd"], "dd/MM/yyyy"))
        self.date_ngaykt.setDate(QDate.fromString(data["ngaykt"], "dd/MM/yyyy"))