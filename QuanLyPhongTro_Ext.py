...
import sys
import json
from PyQt6.QtGui import QPixmap # gắn ảnh đại diện
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def format_profile(self):
        pixmap = QPixmap("icons/profile.png")  # đường dẫn ảnh
    ...
    def format_main(self):
        ...
        self.btn_addprofile.setIcon(QIcon("icons/downloads.png"))
...
#================TIỆN ÍCH DÙNG CHUNG ===========================================================================================
    ...
    def load_info_admin(self):
        # load combobox
        self.cbo_pf_gtinh.addItems(["Nam", "Nữ"])

        with open("data/info_admin.json", "r", encoding="utf-8") as f:
            data = json.load(f)

        admin: dict[str, str] = data

        # Set dữ liệu lên UI
        self.lne_pf_hoten.setText(admin["hoten"])
        self.cbo_pf_gtinh.setCurrentText(admin["gioitinh"])

        date = QDate.fromString(admin["namsinh"], "yyyy-MM-dd")
        self.date_pf_namsinh.setDate(date)

        self.lne_pf_cccd.setText(admin["cccd"])
        self.lne_pf_sdt.setText(admin["sodienthoai"])
        self.lne_pf_diachi.setText(admin["diachi"])
        self.lne_pf_stk.setText(admin["sotaikhoan"])
        self.lne_pf_tendnhap.setText(admin["tendangnhap"])
        self.lne_pf_pass.setText(admin["matkhau"])

        # Chỉ đọc không được chỉnh sửa
        for lne in [
            self.lne_pf_hoten,
            self.lne_pf_cccd,
            self.lne_pf_sdt,
        ...
        for cbo_date in [
            self.cbo_pf_gtinh,
            self.date_pf_namsinh
        ]:
            cbo_date.setEnabled(False)
    ...
    def load_recent_requests(self):
        ...
        # xoá item cũ
        for i in reversed(range(self.layout_recent_requests.count())):
            widget = self.layout_recent_requests.itemAt(i).widget()
            if widget:
                widget.deleteLater()
        ...
        # lấy 4 request sớm nhất
        def parse_date(d):
            return datetime.strptime(d, "%d/%m/%Y")
        sorted_data = sorted(data, key=lambda x: parse_date(x["ngaygui"]))
        recent_data = sorted_data[:4]


        # trạng thái
        status_map = {
            "chưa xử lý": ("#ef4444", "Chưa xử lý"),
            "đang xử lý": ("#3b82f6", "Đang xử lý"),
            "đã xử lý": ("#22c55e", "Đã xử lý"),
        }
        ...
        for item in recent_data:
            ...
            # dấu chấm xanh cho đã xử lý, đỏ cho chưa xử lý
            dot = QLabel()
            dot.setFixedSize(10, 10)

            color, text = status_map.get(item["trangthai"].lower(), ("#999", "Không rõ"))
            ...
            # chỉ lấy phòng và tên
            info = QLabel(f'{item["khachthue"]} - Phòng {item["sophong"]}')
            info.setStyleSheet("font-size:14px; color:black; font-weight: bold")  # ép chữ đen luôn
            ...
            self.layout_recent_requests.addWidget(row_widget)
