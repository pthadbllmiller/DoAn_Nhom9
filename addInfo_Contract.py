...
# lấy font arial để không bị lỗi tiếng việt
...
class addInfo_Contract:
    def add_contract(self):
        ...
        if any(c.get("sophong") == so_phong and c.get("trangthai", "").strip().lower() == "Hiệu lực"
            for c in contracts):
   ...
   # hàm có thể chỉnh sửa
    ...
    def delete_contract(self):
       ...
       # xoá bằng cách chỉ giữ lại những hợp đồng không phải là hợp đồng đã chọn
        ...
        with open("data/contracts.json", "w", encoding="utf-8") as f:
            json.dump(new_contracts, f, ensure_ascii=False, indent=4)
        ...
        for i, c in enumerate(contracts):
            if c["maHD"] == contract_id:
                ...
                contracts[i] = {
                    ...
                    "sophong":            self.lne_sophong_2.text().strip(),
                    "dientich":           self.lne_dientich_2.text().strip(),
                    "mucdich":            self.lne_mucdich_2.text().strip(),
                    "trangthai":          self.cbo_trangthai_2.currentText(),
                    "makh":               c.get("makh", ""),
                    "hoten":              self.lne_hoten_4.text().strip(),
                    "gioitinh":           self.cbo_gioitinh_4.currentText(),
                    "ngaysinh":           self.date_namsinh_4.date().toString("dd/MM/yyyy"),
                    "cccd":               self.lne_cccd_3.text().strip(),
                    "diachi":             self.lne_dcthuongtru_4.text().strip(),
                    "ngaybd":             self.date_ngaybd_2.date().toString("dd/MM/yyyy"),
                    "ngaykt":             self.date_ngaykt_2.date().toString("dd/MM/yyyy"),
                    "giathue":            self.lne_giathue_2.text().strip(),
                    "tiencoc":            self.lne_tiencoc_2.text().strip(),
                    "ngaythanhtoan":      self.lne_ngaythanhtoan_2.text().strip(),
                    "hinhthucthanhtoan":  self.lne_htthanhtoan_2.text().strip(),
                }
                break
    ...
    def export_contract_pdf(self):
        ...
        # chọn nơi lưu file
        file_path, _ = QFileDialog.getSaveFileName(self, "Lưu file PDF", f"Thông tin hợp đồng {contract_id}.pdf", "PDF Files (*.pdf)")
        if not file_path:
            return
        ...
        # tiêu đề
        c.setFont("Arial", 16)
        c.drawString(150, y, "HỢP ĐỒNG THUÊ PHÒNG")
        y -= 40

        c.setFont("Arial", 12)

        # thông tin hợp đồng
        draw_line(f"Mã hợp đồng: {self.lne_mahopdong_2.text()}")
        draw_line(f"Ngày tạo: {self.date_ngaytao_2.date().toString('dd/MM/yyyy')}")
        draw_line(f"Số phòng: {self.lne_sophong_2.text()}")
        draw_line(f"Diện tích: {self.lne_dientich_2.text()} m2")
        draw_line(f"Mục đích: {self.lne_mucdich_2.text()}")
        draw_line(f"Trạng thái: {self.cbo_trangthai_2.currentText()}")

        y -= 10
        draw_line("---- Người thuê ----")
        draw_line(f"Họ tên: {self.lne_hoten_4.text()}")
        draw_line(f"CCCD: {self.lne_cccd_3.text()}")
        draw_line(f"Địa chỉ: {self.lne_dcthuongtru_4.text()}")

        y -= 10
        draw_line("---- Thông tin thuê ----")
        draw_line(f"Ngày bắt đầu: {self.date_ngaybd_2.date().toString('dd/MM/yyyy')}")
        draw_line(f"Ngày kết thúc: {self.date_ngaykt_2.date().toString('dd/MM/yyyy')}")
        draw_line(f"Giá thuê: {self.lne_giathue_2.text()}")
        draw_line(f"Tiền cọc: {self.lne_tiencoc_2.text()}")
        draw_line(f"Ngày thanh toán: {self.lne_ngaythanhtoan_2.text()}")
        draw_line(f"Hình thức thanh toán: {self.lne_htthanhtoan_2.text()}")

        c.save()

        QMessageBox.information(self, "Thông báo", "Xuất PDF thành công!")
